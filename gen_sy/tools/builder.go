...
func calcFullCodeByDiv(div []string, mappings map[string]string, strokeTable map[string]string) (full string, code string) {
	...
	if len(div) == 1 {
		...
		// 第三码取第二部件大码（如果第一部件只有双编码，则取第二部件中码；如果第二部件只有双编码，则取第二部件小码）
		if len(firstCompCode) > 2 {
			code += secondCompCode[:1]
		} else if len(firstCompCode) == 2 {
			code += secondCompCode[1:2]
		}
		
		// 第四码取第二部件中码（如果第一部件只有双编码，则取第二部件小码；如果第二部件只有双编码，则第四码为空）
		if len(firstCompCode) > 2 {
			code += secondCompCode[1:2]
		} else if len(firstCompCode) == 2 && len(secondCompCode) > 2 {
			code += secondCompCode[2:3]
		}
		
		// 第五码取第二部件小码（如果第一部件只有双编码，则第五码为空）
		if len(firstCompCode) > 2 && len(secondCompCode) == 3 {
			code += secondCompCode[2:3]
		}
	} else if len(div) == 3 {
		// 三根字
		firstCompCode := mappings[div[0]]
		secondCompCode := mappings[div[1]]
		thirdCompCode := mappings[div[2]]
		
		if len(firstCompCode) == 0 || len(secondCompCode) == 0 || len(thirdCompCode) == 0 {
			return "", ""
		}
		...
		// 第三码取第二部件大码（如果第一部件只有双编码，则取第三部件大码）
		if len(firstCompCode) > 2 {
			code += secondCompCode[:1]
		} else {
			code += thirdCompCode[:1]
		}
		
		// 第四码取第三部件大码（如果第一部件只有双编码，则取第三部件中码）
		if len(firstCompCode) > 2 {
			code += thirdCompCode[:1]
		} else {
			code += thirdCompCode[1:2]
		}
		
		// 第五码取第三部件中码（如果第一部件只有双编码，则第五码取第三部件小码）
		if len(firstCompCode) > 2 {
			code += thirdCompCode[1:2]
		} else if len(firstCompCode) == 2 && len(thirdCompCode) > 2 {
			code += thirdCompCode[2:3]
		}
	} else {
		// 多根字
		firstCompCode := mappings[div[0]]
		...
		if len(firstCompCode) == 0 || len(secondCompCode) == 0 || len(thirdCompCode) == 0 || len(lastCompCode) == 0 {
			return "", ""
		}
		...
		// 第四码取第三部件大码（如果第一部件只有双编码，则取末部件大码）
		if len(firstCompCode) > 2 {
			code += thirdCompCode[:1]
		} else {
			code += lastCompCode[:1]
		...
		// 第五码取末部件大码（如果第一部件只有双编码，则取末部件中码；如果末部件只有双编码，则取末部件小码）
		if len(firstCompCode) > 2 {
			code += lastCompCode[:1]
		} else if len(firstCompCode) == 2 {
